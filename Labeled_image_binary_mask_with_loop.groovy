// Hyper parameters
String classifier_name = "Stroma_240625" // Pixel classifier name (CD8_full, PANCK_240213, Stroma_240625, PicroSiriusRed_240422) 
String label_name = "Stroma" // Annotation selection name (Immune cells, Tumor, Stroma)
double minimum_object_size = 50 // Minimum size for created objects
double minimum_hole_size = 1 // Minimum whole size inside a detected object else it will be filled
double downsample = 1 // Define how much to downsample during export (may be required for large images)
int tiling_factor = 50 // Size of image to size of the tile
String format = "png" // Mask format
String name = "Stroma" // Name of the mask (CD8, Tumor, Stroma, Collagen)
...
def img_name = GeneralTools.stripExtension(imageData.getServer().getMetadata().getName())
...
print "Classifying the slide using " + classifier_name
counter = 1
for (int y = 0; y < maximum_height ; y += tile_height) {
    for (int x = 0; x < maximum_width; x += tile_width) {
        ...
        // file name
        def file_name = img_name + "_" + name + "_" + "mask" + "_" + counter + "_of_" + tiling_factor + "." + format
        ...
        if (new File(outputDir, file_name).exists()) {
            ...
            print tile_number + "Creating annotations"
