...
// Get all micordisections
def micordisections = getAnnotationObjects()


// Define the label (path class) you want to set
def label_positive = getPathClass("Positive")
def label_negative = getPathClass("Negative")

// Loop through the micordisections
for (int i = 0; i < micordisections.size(); i++) {
    // Get the annotation
    def micordisection = micordisections[i]
    String micordisection_name = micordisection.getName().split(" ")[0]

    // Set the label
    print "Setting label " + label_positive + " to " + micordisection_name
    micordisection.setPathClass(label_positive)

    print "Classifying the slide wrt " + micordisection_name
    counter = 1
    for (int y = 0; y < maximum_height ; y += tile_height) {
        for (int x = 0; x < maximum_width; x += tile_width) {
            ...
            if (new File(outputDir, file_name).exists()) {
                ...
                .addLabel("Positive", 255)      // Choose output labels (the order matters!)
    ...
    // Set the label to negative
    print "finished and setting label " + label_negative + " to " + micordisection_name
    micordisection.setPathClass(label_negative)
}
